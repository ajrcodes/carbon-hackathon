...
class FinalizedGroupViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func createGroupFromAPI(){
        if(userGroup.users.count == 0){
            let alert = UIAlertController(title: "Error", message: "No users are in group, you need to have at least 2 users", preferredStyle: UIAlertControllerStyle.alert)
            alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.default, handler: nil))
            self.present(alert, animated: true, completion: nil)
        }
        else if(userGroup.users.count == 1){
            let alert = UIAlertController(title: "Error", message: "Only one user is in your group, you need add at least 2 users", preferredStyle: UIAlertControllerStyle.alert)
            alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.default, handler: nil))
            self.present(alert, animated: true, completion: nil)
        }
        else{
